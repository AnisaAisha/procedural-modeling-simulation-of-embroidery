...
MOTIF_KEY = "5" #change this key for motif needed
...
def smooth_random_field(w, h, low_res, rng):
    ...
    # Resizes the noise to exactly match the input image's dimensions
...
def process_image(image_path, normal_strength):
    ...
    # Dynamic background detection (samples top-left pixel)
    bg_color = rgb[0, 0]
    is_bg = np.all(np.abs(rgb - bg_color) < 0.05, axis=-1)
    
    # FIX APPLIED: Mask out transparent pixels/background and cast directly to uint8
    mask = ((alpha > 10) & (~is_bg)).astype(np.uint8)
    
    if mask.sum() == 0:
        print(f"Warning: No non-background regions detected in {image_path}. Using entire image.")
    ...
    # OpenCV connectedComponents now safely receives a uint8 mask
    ...
    # UPDATED: Use (h, w) to match the original image aspect ratio
    ...
    # UPDATED: Pass w and h to the noise generator
